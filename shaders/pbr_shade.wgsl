...
// ADR-204. Included here, so the one file every shading path goes through is also the one
...
fn shadeSurface(worldPos: vec3<f32>, normalIn: vec3<f32>, uv: vec2<f32>, frontFacing: bool,
    ...
    if (object.flags.z > 0.5) { // unlit
        // ADR-204: additive, even here. An unlit surface is one the lighting does not reach, not one
    ...
    if (frame.lightCounts.z > 0.5) {
        ...
        // ADR-204: the world-effect contribution, added to the radiance *before* fog so a wave far
    ...
    // ADR-204, as on the styled path above: additive, pre-fog, and into the emission target.
    let fx = worldEffectsAt(worldPos, n, emissive);
