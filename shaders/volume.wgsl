...
struct VolumeUniforms {
    ...
    heightFog: vec4<f32>, // ADR-568: x = fogUpperDensity, y = fogHeightCurve; ADR-705: z = horizonDensity, w = 0
...
fn fs_volume(in: FsIn) -> @location(0) vec4<f32> {
    ...
    for (var i = 0; i < steps; i = i + 1) {
        ...
        if (vol.heightFog.z > 0.0) {
            fogDensity = fogDensity * (1.0 + vol.heightFog.z * t * 0.001);
        }
