// World effects (ADR-204): the per-fragment half of a spatial phenomenon propagating through the
...
//
// The data comes from `frame.worldEffects`, packed by world::packWorldEffect; the struct here and
...
const kWorldEffectTau: f32 = 6.28318531;
...
// fourth.
fn worldEffectHash3(p: vec3<f32>) -> vec3<f32> {
...
fn worldEffectLuma(c: vec3<f32>) -> f32 { return dot(c, vec3<f32>(0.2126, 0.7152, 0.0722)); }
...
fn worldEffectRainbow(t: f32, saturation: f32, brightness: f32) -> vec3<f32> {
    let c = 0.5 + 0.5 * cos(kWorldEffectTau * (vec3<f32>(t) + vec3<f32>(0.0, 0.33333, 0.66667)));
    return mix(vec3<f32>(1.0), c, clamp(saturation, 0.0, 1.0)) * max(brightness, 0.0);
}

struct WorldEffectResult {
...
//
// Three weights and a derived groundness, not a named category per asset -- see ADR-204. What the
...
fn worldEffectGain(response: vec4<f32>, normal: vec3<f32>) -> f32 {
...
fn worldEffectsAt(worldPos: vec3<f32>, normal: vec3<f32>, emission: vec3<f32>) -> WorldEffectResult {
    var out: WorldEffectResult;
    out.radiance = vec3<f32>(0.0);
    out.bloom = 0.0;
    // Uniform across the draw, so a frame with no effects executes this compare and nothing else.
    let count = u32(frame.worldEffectCount.x + 0.5);
    if (count == 0u) {
        return out;
    }
    let viewDistance = distance(frame.cameraPos.xyz, worldPos);
    for (var i: u32 = 0u; i < count; i = i + 1u) {
        let e = frame.worldEffects[i];
        ...
        if (rings > 0.0) {
            ...
            body = body * (0.55 + 0.45 * cos(kWorldEffectTau * rings * phase));
        }
        ...
        if (e.edge.w > 0.5) {
            tint = tint * worldEffectRainbow(d * e.rainbow.x + e.rainbow.y, e.rainbow.z, e.rainbow.w);
        }
        ...
        if (e.sparkle.x > 0.0 && edgeBand > 2.0e-3) {
            ...
            if (fade > 0.0) {
                ...
                let twinkle = 0.45 + 0.55 * sin(kWorldEffectTau * (r.x + e.sparkle.w));
                spark = blob * max(twinkle, 0.0) * e.sparkle.z * edgeBand * fade;
            }
        }

        let gain = worldEffectGain(e.response, normal);
    ...
    out.bloom = clamp(worldEffectLuma(out.radiance) * 0.6, 0.0, 1.0);
    return out;
}
