...
// Pass 0 (sys.passIndex == 0) renders into `trail`, which persists across frames so the shader can
// sample its own previous contents. Pass 1 renders `trail` to the output.
// sys.audio2 = (lowMid, highMid, onset, beatPhase); beatPhase runs 0..1 between beats.

fn blobPosition() -> vec2<f32> {
    // The blob jumps around the orbit ellipse once per beat and eases in on each beat.
    let phase = sys.audio2.w;
    let eased = 1.0 - (1.0 - phase) * (1.0 - phase);
    let beatIndex = sys.beat.y;
