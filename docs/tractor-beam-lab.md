...
tools/make_tractor_beam_lab.py --legacy \
    examples/world/tractor-beam-lab-legacy.scene.json   # the control: the pre-ADR-260 scenario
...
`--debug-draw` is new with ADR-260 and is the other half of the lab. The overlays existed -- the
...
| beam axis | a **vertical** line through it | world | `direction` is *not* transformed by `applyParameters`, so the column is world-down however the craft banks. Load-bearing; see ADR-260 §4 |
...
Two of those are not where anybody had been looking, and both are in ADR-260: the emitter is not the
