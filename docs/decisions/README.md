...
| [272](ADR-272-a-joint-transform-is-entity-local-and-nothing-had-to-choose.md) | A joint transform is entity-local, and until something implemented it nothing had to choose (`ISkeletonQuery` implemented, `setSkeleton` called, `socketTransform` reports its fallback; the node scale it never carried) | Accepted |
| [273](ADR-273-a-lab-case-that-cannot-be-run-says-what-it-is-waiting-for.md) | A lab case that cannot be run says what it is waiting for (the Character Intelligence Lab: `blockedBy`, the floating hero that read as "obstacles do not contain a character", and the explorer's 1.36 m scrub) | Accepted |
| [274](ADR-274-a-star-is-project-data-and-never-reached-a-render.md) | A star is project data, and until now it never reached a render (session 1 hero, reloaded 0; the third instance of ADR-207's family, and the directed cut it left aimed at nothing) | Accepted |
