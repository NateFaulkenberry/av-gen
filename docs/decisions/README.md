...
| [272](ADR-272-ninety-seconds-is-not-an-amount-of-work.md) | Ninety seconds is not an amount of work (`SeekBudget` prices a scrub in body-steps; the four seek defects; the replay's last step lands on the target and the residual goes to zero; a flatten's texture version becomes conditional) | Accepted |
