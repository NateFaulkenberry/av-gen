...
| [340](ADR-340-a-path-tracer-is-not-what-this-codebase-has-always-called-offline.md) | A path tracer is not what this codebase has always called "offline" (six ADRs already spend that word on a WebGPU quality tier and the ADR-020 batch pipeline -- and `app::RenderJob` renders at `QualityTier::Realtime` -- so the Embree renderer is named for its technique: `avgen::pathtrace` in `src/pathtrace/`; the existing tinyexr writer is extended to named AOV layers rather than adding OpenEXR, because the pinned header already exposes `EXRHeader`/`SaveEXRImageToFile`; `app::JobSystem` cannot serve as the tracer's parallelism -- two hard-coded workers and no parallel-for -- so the BVH commits through `rtcJoinCommitScene` on AV Gen-owned threads; and Embree 4.4.0 does not compile under Apple clang 21 unless `CMAKE_CXX_STANDARD` is unset around it, because the project's global C++23 overrides Embree's own `-std=c++11`) | Accepted |
| [341](ADR-341-the-gltf-brdf-is-kept-faithful-and-it-gains-energy-at-grazing.md) | The glTF BRDF is kept faithful, and it gains 68% of its energy at grazing (the white-furnace test measures a white smooth dielectric at 1.676 directional albedo, confirmed real rather than Monte Carlo by two estimators agreeing to four decimals; metals conserve at 0.9988, so GGX/Smith/Fresnel are right and the fault is the spec's own `(1 - F(v.h))` diffuse suppression, which cannot see how much the specular lobe already took; section 19 asks for the glTF model as the baseline so it stays faithful and the behaviour is pinned as a band rather than fixed inside a Phase 2 commit) | Accepted |
