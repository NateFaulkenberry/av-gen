# ADR-208: What world effects cost, and the cross-binary number that is not evidence

Status: Accepted

## Context / Problem

ADR-207 put a loop over up to eight propagating waves inside `pbr_shade.wgsl`, which is the fragment
...
hundredth of a millisecond. That is the term being where ADR-207 said it was.
...
* A per-cluster effect list (ADR-207's own revisit trigger), which would change the cost model from
