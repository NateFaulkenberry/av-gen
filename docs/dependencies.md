...
| Embree (vendors `sse2neon.h` for the NEON path) | v4.4.0 | Apache-2.0 (Embree); sse2neon.h is MIT | ray/triangle intersection, BVH build and occlusion queries for the CPU path tracer (`src/pathtrace/`) | the reference CPU intersection kernel; ADR-351. AV Gen keeps scene representation, materials, sampling, the integrator, AOVs and output, so only intersection is delegated (spec section 74). Built with `EMBREE_TASKING_SYSTEM=INTERNAL` so oneTBB is not a dependency, and committed via `rtcJoinCommitScene` from AV Gen-owned threads so Embree starts no thread pool of its own. **`CMAKE_CXX_STANDARD` must be unset around its `CPMAddPackage`**: the project's global C++23 overrides Embree's own `-std=c++11` and Embree 4.4.0 does not compile as C++23 under Apple clang 21 |
