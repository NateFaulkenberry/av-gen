...
| 3 | **shadow** | cascade transitions, swimming, popping, bias, filtering | yes | no — and ⚠ **there now IS a shadow AOV** (`--aov shadow`, ADR-255/258), so the residual can be gated on shadowed pixels. It recomputes the term rather than capturing it, 0.43% of pixels differing from the lit pass's own, so a shadow scene's numbers carry that qualification |
