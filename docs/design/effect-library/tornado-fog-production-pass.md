...
| **§25 colour** (scattering/absorption/height/distance colour) | **height and distance colour done (ADR-714)**: luminance-preserving tints of the glow, weighted by height and by camera distance, never by density. Scattering/absorption colour not built. | `shaders/fog.wgsl`, `src/world/fog_field.cpp`, `volumetric_fog_effect.cpp` | A scattering colour would make the march's scalar `mediumScatter` a `vec3` (every medium kind). |
| **§16 flow controls** | **done (ADR-713)**: Swirl (the dead "Drift" row, made live), Swell as expansion/contraction (dead until now), Turbulence = curl (a divergence-free displacement of the whole field). Dissipation and one-shot expansion are keyframes of Fog density / Bank radius, so no row. The audit also found and cut five dead noise rows (Billow, Churn, Wisps, Wisp scale, Fine detail). | `shaders/fog.wgsl`, `src/world/fog_field.cpp`, `volumetric_fog_effect.cpp`, both bound twins | Turbulence at scale 4 is grainy at 32 steps (ADR-713's table); a band-limit is the follow-up if that end of the range is wanted. |
