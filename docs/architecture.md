...
| `assets` | avgen_core | image decode/encode (stb), glTF 2.0 import (fastgltf) into a `Scene`, `AssetRegistry` (cached, versioned, path-resolving loads), `VideoWriter` (AVFoundation native / external ffmpeg) | fastgltf, stb, AVFoundation (macOS) |
...
| `app` | avgen | `Engine` (the pipeline; also compiled into the test binary), `Application` (live/headless loops, CLI), `RecentFiles`, `RenderSettings`/`RenderJob` (offline renders: own offline engine + renderer, encoder threads, video writer; ADR-020) | everything |
