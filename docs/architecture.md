...
```
  ...
  │   └ pass 3: FrameOverlay → the 2D composition over the tone-mapped frame (ADR-081)
...
| `comp` | avgen_core | the 2D composition above the render (ADR-081): `LayerStack` (ordered layers, cached geometry, per-frame items), `Layer` with `TextLayer`/`ShapeLayer`, `FontBackend` (CoreText shaping and outline rasterisation), `GlyphAtlas` (one shared signed-distance atlas keyed by face and glyph, not by size) | glm, params, CoreText (macOS) |
