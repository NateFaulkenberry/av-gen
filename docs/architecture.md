...
| `ai` | avgen_core | AI: `InferenceBackend`/`InferenceService` (image models; ADR-065) and, beside it, the **control plane** (ADR-094) — `ToolRegistry` (the engine's semantic interface: named, schema-validated operations over real engine systems), `engine_tools` (34 tools over `ParameterSet`, `Timeline`, `Modulator`, `SignalBus`, `Scene`), `SnapshotStore`/`TransactionSink`, `Provider` + the vendor adapters, `MainThreadQueue`, `Orchestrator` (the agent loop) and `ControlPlane`. No render, audio or GPU dependency; the AI is one client of the tool registry and an MCP server or script API could be another | params, scene, signals, app::Engine (header), Security + Foundation (macOS) |
...
| `ui` | avgen_platform | `ImGuiLayer` (SDL3 + WebGPU backends), `ControlPanel` (transport, response, generated parameter panel, analysis plots, performance), `CompositionPanel` (the layer stack, the text/shape inspector and the key dots; ADR-083), `AiPanel` (the prompt and the structured activity stream) and `SettingsPanel` (application-level settings incl. AI providers and credentials; ADR-094) | ImGui, ImPlot |
...
| job workers (`JobSystem`, ADR-064) | a job's own state | nothing the frame loop owns, except through an explicit hand-off |
| AI agent loop (a job worker; ADR-094) | the conversation, the HTTP request | **no engine state at all** — every tool body is marshalled onto the main thread through `ai::MainThreadQueue`, which the frame loop drains under a time budget |
