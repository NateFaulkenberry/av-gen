...
| **11** | **Metal backend, or defer** — **deferred on the measurement** | Gated on 10, and 10 says not yet. The 136 ms figure also prices the audit's other option: BVH reuse across frames in the Embree backend — **built, ADR-583**: a range now rebuilds only what changed, so on the Tree of Life the per-frame build falls from a full rebuild to the top level plus an exact compare. |
