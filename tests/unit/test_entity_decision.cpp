...
TEST_CASE("a misspelled considerer is refused by name", "[entity][decision][adr333]") {
    ...
    // Four in ADR-333, five since ADR-335 added `route`, seven since Phase D added `react` and
    // `social`, eight with `goal`. The count is asserted rather than the list because the loop above already checks
    // every name; this catches a kind added to the factory and not to the vocabulary, which is the
    // direction the failure actually goes.
    CHECK(considererKinds().size() == 8);
}
