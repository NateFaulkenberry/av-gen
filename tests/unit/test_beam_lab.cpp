...
TEST_CASE("A lifted animal is drawn on the beam's axis, at every position, facing and asset",
    ...
    // sideways part and an up-and-down part, and until ADR-260 only the sideways part had ever been
    // asked.
    for (const Lift& l : run.lifts) {
        ...
        // top, and before ADR-260 the animal was under the one and over the other.
...
// build, carrying the scenario **as it was authored before ADR-260**: `clearance` on the two
...
TEST_CASE("The pre-ADR-260 scenario fails every one of those checks", "[stage][beam][lab][control]") {
    if (!farmAssetsPresent()) {
        SKIP("assets/farm is not present (the GLBs are gitignored; run tools/make_farm_animals.sh)");
    }
