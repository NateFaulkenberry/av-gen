// Phase 1 of the path tracer, end to end (ADR-351): snapshot, BVH, primary rays, Lambertian direct
...
TEST_CASE("the capability report warns about the BRDF's grazing energy gain on every render",
    ...
    // ADR-352. The owner chose to keep the glTF model faithful, gain and all, so the gain is
    ...
    const bool found = std::any_of(
        ...
        [](const std::string& c) { return c.find("ADR-352") != std::string::npos; });
    REQUIRE(found);
    ...
    REQUIRE(report.find("ADR-352") != std::string::npos);
