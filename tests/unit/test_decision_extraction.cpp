// The byte-identical control for the `Explore` goal-model extraction (ADR-333 §3).
//
// `Explore` is the only autonomous mind this engine has ever had and five Glowmere characters
// depend on it. ADR-333 moved its goal model -- taste over the interest registry, damped by
...
TEST_CASE("the extracted goal model walks the same route to the bit", "[entity][decision][adr333]") {
    if (!assetsPresent()) {
    ...
    if (std::getenv("AVGEN_WRITE_EXPLORE_GOLDEN") != nullptr) {
        ...
        out << "# Sixty seconds of examples/labs/character/character-intelligence-lab.scene.json at a\n"
               ...
               "# its raw 32 bits. Written by the build BEFORE ADR-333's goal-model extraction and\n"
...
TEST_CASE("the trace is sensitive to the goal model it is measuring", "[entity][decision][adr333]") {
    if (!assetsPresent()) {
        WARN("assets missing; skipping");
        return;
    }
