// Two-bone foot IK (ADR-359): the solver on its own, then the layer on the rigs that have a leg.
//
...
TEST_CASE("a foot layer moves one hoof and nothing else on the bull", "[ik][layers][farm]") {
    ...
    // 0.12 m ABOVE where the hoof stands, and the direction is the whole point (ADR-359 SS5). This
