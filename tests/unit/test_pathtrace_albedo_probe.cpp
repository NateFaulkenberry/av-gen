// The directional-albedo probe (ADR-345, owner's request).
//
...
// A bright, smooth, white, non-metallic room seen at glancing angles -- the exact case ADR-345
...
TEST_CASE("directionalAlbedoAt computes the integral, and agrees with the white-furnace numbers",
          "[unit][pathtrace][probe]") {
    // The probe must measure the same quantity ADR-345 measured, or its output is not comparable to
    ...
    REQUIRE(a > 1.5f);   // the ADR-345 band
    REQUIRE(a < 1.8f);
...
TEST_CASE("the probe finds the gain, names the material, and records angle and depth",
    ...
    // `worstAlbedo` is a confident LOWER bound, so it must sit at or below ADR-345's measured 1.68
    ...
    REQUIRE(text.find("ADR-345") != std::string::npos);
