// Glowmere Valley 3: the autonomous-character showcase (ADR-344).
//
...
TEST_CASE("Glowmere Valley 3: the watcher attends to another character", "[glowmere3][entity]") {
    ...
    // squeezed. Before `ground` learned to declare a body (ADR-344), this measured **0.238 m** --
...
// emitters are the size the scene authored, and the two engine-level opt-ins ADR-344 added are
...
TEST_CASE("Glowmere Valley 3 runs the world its scene describes, loaded as a project",
    ...
    // The two opt-ins ADR-344 added are registered and on, which is the difference between a
