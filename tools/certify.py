...
THE VISUAL GATE (G4)
--------------------
`--capture DIR` writes one frame per subject into DIR at a fixed second, and if DIR already holds a
capture from a previous run it prints the pixel difference between them.
...
the check (the same reasoning `examples/qa/baselines` gives for recording derived *state* instead of
pixels).  What a run-to-run diff answers is the question actually worth asking during an upgrade:
*this* change, did it move the image, and where.  §50 of the spec then requires a person to look at
the frame -- a diff of zero and a diff of eight million both need an eye on the picture, and this
tool only ever says which one happened.

WHAT IT DOES NOT DO
-------------------
It does not fail on a timing, and it does not fail on a pixel.  It fails on a counter, on a GPU
error, on a dirty tree when `--strict` is given, and on nothing else.
"""
...
def capture_and_diff(subject: dict, args) -> list[str]:
    """One frame, and the difference from the frame the last run left here."""
    declaration = subject["declaration"]
    target = args.capture / f"{subject['name']}.ppm"
    previous = args.capture / f"{subject['name']}.previous.ppm"
    if target.is_file():
        target.replace(previous)
    # A fixed second, reached by rendering to it at a fixed step: the capture has to be the same
    # frame of the same second every run or the diff is measuring the clock.
    frames = max(int(round(args.capture_second * declaration.get("fps", 30))), 1)
    command = [str(BINARY), "--headless", f"--{declaration.get('load', 'generate')}",
               str(subject["path"]), "--frames", str(frames),
               "--fps", str(declaration.get("fps", 30)),
               "--size", declaration.get("atResolution", "1280x800"),
               "--tier", declaration.get("tier", "realtime"), "--capture", str(target)]
    if not args.no_lock:
        command = [str(GPU_LOCK)] + command
    completed = subprocess.run(command, cwd=ROOT, capture_output=True, text=True)
    if completed.returncode != 0 or not target.is_file():
        print(f"  {subject['name']}: capture FAILED")
        return [f"- **{subject['name']}**: capture failed", ""]
    if not previous.is_file():
        print(f"  {subject['name']}: captured {target.name} (no previous capture to diff against)")
        return [f"- **{subject['name']}**: `{target.name}` captured; no previous frame to diff", ""]
    verdict = ppm_diff(previous, target)
    print(f"  {subject['name']}: {verdict}")
    return [f"- **{subject['name']}**: {verdict}", ""]


def read_ppm(path: pathlib.Path) -> tuple[int, int, bytes]:
    """Binary P6, which is what --capture writes. Header fields may be split across whitespace and
    comments may appear between them, so the header is tokenised rather than split on newlines."""
    ...
    return fields[0], fields[1], data[offset + 1:]


def ppm_diff(a: pathlib.Path, b: pathlib.Path) -> str:
    wa, ha, pa = read_ppm(a)
    wb, hb, pb = read_ppm(b)
    if (wa, ha) != (wb, hb):
        return f"size changed: {wa}x{ha} -> {wb}x{hb}"
    differing, worst = 0, 0
    total = wa * ha
    for i in range(total):
        d = max(abs(pa[i * 3 + k] - pb[i * 3 + k]) for k in range(3))
        if d:
            differing += 1
            if d > worst:
                worst = d
    if differing == 0:
        return "byte-identical to the previous capture"
    return (f"{differing:,} of {total:,} pixels differ ({100.0 * differing / total:.2f}%), "
            f"largest channel delta {worst} -- LOOK AT THE FRAME")
...
def main() -> int:
    ...
    parser.add_argument("--capture", type=pathlib.Path,
                        help="write one frame per subject here and, if a previous capture is "
                             "present, diff against it (G4). Look at the frames afterwards: §50 "
                             "asks for an eye on the image, not only a difference count")
    parser.add_argument("--capture-second", type=float, default=2.0,
                        help="the second to capture at; fixed so two runs capture the same frame")
    ...
    if args.capture:
        print("\n=== the visual gate: one frame per subject, against the previous capture ===")
        args.capture.mkdir(parents=True, exist_ok=True)
        for subject in subjects:
            lines.extend(capture_and_diff(subject, args))
