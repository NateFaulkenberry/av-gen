...
struct RenderStats {
    ...
    // ADR-344: what the entity LOD selector did this frame. Separate from `lodCounts`, which is the
...
private:
    ...
    // ADR-344: one mesh's coarser rungs on the GPU, and what the selector needs to cost them.
    ...
    // ADR-344: parallel to `meshes_`, by MeshId. A mesh with no chain carries an empty GpuMeshLod,
