...
struct RenderStats {
    ...
    std::uint32_t worldEffects = 0; // ADR-204: effects live in the frame block this frame
};
...
struct FrameUniforms {
    ...
    // ADR-204: the world effects this frame. Appended *after* `lights` so no offset above moved,
...
public:
    ...
    struct PassToggles {
        ...
        // ADR-204. Off: the frame block reports zero world effects, so the per-fragment loop
