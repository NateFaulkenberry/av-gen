...
void PostProcessor::resetExposure() {
    ...
    //
    // ADR-385: EVERY slot, and the write cursor too. Clearing one of two would leave the ring
...
bool PostProcessor::takeMeasurement(float& luminance) {
    // ADR-385: read the slot `encodeMetering` is about to overwrite. With two slots that is the one
...
void PostProcessor::encodeMetering(wgpu::CommandEncoder& encoder, const PostFrameInputs& in, gpu::TransientPool& pool,
    ...
    // proves the mechanism is live rather than the comparison vacuous. See ADR-385.
...
wgpu::TextureView PostProcessor::run(wgpu::CommandEncoder& encoder, const PostFrameInputs& in, gpu::TransientPool& pool) {
    ...
    if (autoExposure) {
        ...
        // Manual: nothing is metered, so nothing may be left in flight. ADR-385 -- every slot, not
