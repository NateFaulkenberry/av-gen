...
GroundResult GroundFollower::update(const Navigator& nav, glm::vec2 p, float yaw, float speed,
    ...
    // ground the footprint covers, which is what gives a foot IK layer room to lift from (ADR-359).
    ...
    out.pitch = pitch_;
    out.roll = roll_;
    // The surface, for anything that has to stand *on* it rather than lean with it (ADR-359). The
