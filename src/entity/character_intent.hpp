...
// **One producer: the `motionScript` behaviour** (Phase C §65). Find any others with
// `grep -rn "intent.valid = true" src`. `Entity::advanceMotion` reads it (`if (state_.intent.valid)`
// at `entity.cpp`); a body whose behaviours never set `valid` takes the polar `speed`/`yaw`
// reconstruction underneath it, which is still every body in every shipping scene. ADR-615
// recorded this as dark with no producer; ADR-623 and §65 gave it one.
