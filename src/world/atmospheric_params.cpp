...
// ADR-390: the trailing pair of accessors, and why they exist.
//
// The Cosmic Ocean's ~113 rows are declared in `world/cosmic_ocean.hpp` over `CosmicOcean`, not
// over `AtmosphericEffect` -- the rows belong with the struct they describe, and a hundred and
// thirteen of them landing in this file is exactly what ADR-390's Sequencing section set out to
// avoid. Bridging them needs a per-row thunk from `const Effect&` to `const CosmicOcean&`, and a
// capturing lambda cannot become a function pointer, so the thunks would have to be generated by a
// template over the table's indices -- which requires the table to be visible at compile time,
// which means moving two hundred lines into a header to serve one adapter.
//
// Carrying the ocean's own pointers instead costs two words per row and one branch per access, at
// registration and capture time only. Every existing row leaves them null and is untouched.
struct FloatField {
    const char* leaf;
    float lo, hi, slo, shi;
    float (*getEffect)(const Effect&);
    void (*setEffect)(Effect&, float);
    float (*getOcean)(const CosmicOcean&) = nullptr;
    void (*setOcean)(CosmicOcean&, float) = nullptr;
    [[nodiscard]] float get(const Effect& e) const {
        return getOcean != nullptr ? getOcean(e.cosmicOcean) : getEffect(e);
    }
    void set(Effect& e, float v) const {
        if (setOcean != nullptr) { setOcean(e.cosmicOcean, v); } else { setEffect(e, v); }
    }
};

struct ColorField {
    const char* leaf;
    glm::vec3 (*getEffect)(const Effect&);
    void (*setEffect)(Effect&, glm::vec3);
    glm::vec3 (*getOcean)(const CosmicOcean&) = nullptr;
    void (*setOcean)(CosmicOcean&, glm::vec3) = nullptr;
    [[nodiscard]] glm::vec3 get(const Effect& e) const {
        return getOcean != nullptr ? getOcean(e.cosmicOcean) : getEffect(e);
    }
    void set(Effect& e, glm::vec3 v) const {
        if (setOcean != nullptr) { setOcean(e.cosmicOcean, v); } else { setEffect(e, v); }
    }
};
...
struct BoolField {
    const char* leaf;
    bool (*getEffect)(const Effect&);
    void (*setEffect)(Effect&, bool);
    bool (*getOcean)(const CosmicOcean&) = nullptr;
    void (*setOcean)(CosmicOcean&, bool) = nullptr;
    [[nodiscard]] bool get(const Effect& e) const {
        return getOcean != nullptr ? getOcean(e.cosmicOcean) : getEffect(e);
    }
    void set(Effect& e, bool v) const {
        if (setOcean != nullptr) { setOcean(e.cosmicOcean, v); } else { setEffect(e, v); }
    }
};

// The ocean's tables, adapted once. Built at runtime because `cosmicFloatFields()` hands back a
// span of a table in another translation unit; the vectors are function-local statics, so the cost
// is one pass the first time anything registers an ocean.
const std::vector<FloatField>& cosmicFloats() {
    static const std::vector<FloatField> v = [] {
        std::vector<FloatField> out;
        out.reserve(cosmicFloatFields().size());
        for (const CosmicFloatField& f : cosmicFloatFields()) {
            out.push_back(FloatField{f.leaf, f.lo, f.hi, f.slo, f.shi, nullptr, nullptr, f.get, f.set});
        }
        return out;
    }();
    return v;
}
const std::vector<ColorField>& cosmicColors() {
    static const std::vector<ColorField> v = [] {
        std::vector<ColorField> out;
        out.reserve(cosmicColorFields().size());
        for (const CosmicColorField& f : cosmicColorFields()) {
            out.push_back(ColorField{f.leaf, nullptr, nullptr, f.get, f.set});
        }
        return out;
    }();
    return v;
}
const std::vector<BoolField>& cosmicBools() {
    static const std::vector<BoolField> v = [] {
        std::vector<BoolField> out;
        out.reserve(cosmicBoolFields().size());
        for (const CosmicBoolField& f : cosmicBoolFields()) {
            out.push_back(BoolField{f.leaf, nullptr, nullptr, f.get, f.set});
        }
        return out;
    }();
    return v;
...
std::span<const FloatField> floatFields(AtmosphereKind kind) {
    ...
    case AtmosphereKind::CosmicOcean: return std::span<const FloatField>(cosmicFloats());
...
std::span<const ColorField> colorFields(AtmosphereKind kind) {
    ...
    case AtmosphereKind::CosmicOcean: return std::span<const ColorField>(cosmicColors());
...
std::span<const BoolField> boolFields(AtmosphereKind kind) {
    ...
    case AtmosphereKind::CosmicOcean: return std::span<const BoolField>(cosmicBools());
...
void sanitise(Effect& e) {
    ...
    // ADR-390: the ocean's own belt and braces, which are mostly integer loop bounds -- a modulation
    // curve that drove `planetCells` to 2.3 would be a fractional neighbourhood in the shader.
    sanitiseCosmicOcean(e.cosmicOcean);
}
...
std::vector<params::ModRoute> defaultAtmosphericRoutes(std::string_view effectName, AtmosphereKind kind) {
    ...
    case AtmosphereKind::CosmicOcean:
        // ADR-390 §11's proposed mapping, sparse on purpose: a good Cosmic Ocean is not every
        // effect maximised, and five routes on a sky of a hundred and thirteen parameters is a
        // starting pose an artist edits rather than a preset that has already decided.
        //
        // Every depth is a fraction of the leaf's soft range, as the arms above are: `intensity`
        // reaches 2.0, `nebulaFarDensity` 1.0, `flowTurbulence` 1.0, `starTwinkle` 1.0, and
        // `eventProbability` 1.0. The long releases are what stop a sky flickering -- a nebula that
        // answers a kick drum in 40 ms is a strobe, not weather.
        add("audio.rms", "intensity", 0.22f, 120.0f, 700.0f);
        add("audio.bass", "nebulaFarDensity", 0.10f, 140.0f, 800.0f);
        add("audio.mid", "flowTurbulence", 0.12f, 80.0f, 520.0f);
        add("audio.treble", "starTwinkle", 0.18f, 30.0f, 260.0f);
        add("beat.pulse", "eventProbability", 0.15f, 10.0f, 300.0f);
        break;
    }
    return routes;
}
