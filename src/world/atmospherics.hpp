...
enum class AtmosphereKind : std::uint8_t {
    ...
    // ADR-383. A cosmic vortex is a sky phenomenon a scene AUTHORS, so it belongs in this family
...
// ADR-383: the cosmic vortex as an authored instance rather than a singleton on `Environment`.
...
// ADR-383. A vortex style leaves `center` alone: where the funnel is in the world is a placement
...
struct AtmosphericFrame {
    ...
    // ADR-383: the live vortex, if a scene authored one. ONE, not an array: the volumetric march
...
struct AtmosphericCounts {
    ...
    std::size_t vortices = 0; // ADR-383; at most one is used, the rest count as dropped
