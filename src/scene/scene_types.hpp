...
// ADR-348. Which background the scene pass should draw, as a function of the environment alone.
//
...
enum class SkyBackground : std::uint8_t {
    ...
    Analytic,     // evaluate the procedural sky directly (ADR-348)
...
struct Environment {
    ...
    // ADR-348. Draw the *analytic* sky as the background even when `environmentMap` is bound, so a
