...
Result<void> Composition::setWorldEffects(std::vector<world::WorldEffect> effects) {
    // The whole set or none of it (ADR-204, and the same rule `setHeroes` follows): an effect whose
...
nlohmann::json Composition::toJson() const {
    ...
    // ADR-204. Written only when there are effects, so every scene that never declared one writes
...
Result<std::unique_ptr<Composition>> Composition::fromJsonImpl(const nlohmann::json& j,
    ...
    // ADR-204: the world effects. Read after the heroes, because a `hero` source names one and a
