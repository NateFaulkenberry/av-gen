...
// is assembled.
// ADR-354. `direction` is the direction the light TRAVELS; the angles describe where it comes
...
Result<void> Composition::setAuthoredLights(std::vector<AuthoredLight> lights) {
    ...
    // list a light the scene no longer has (ADR-354).
...
// ADR-354: the authored lights' live knobs. Every light gets intensity, colour and an enable;
...
// ADR-348: this asset's LOD chains, built once and cached. The returned chains' `base` is the
...
void Composition::rebuild() {
    ...
    for (const auto& nodePtr : nodes_) {
        ...
        case NodeKind::Gltf: {
            ...
            // ADR-348: the coarser rungs, when this node asked for them. Built once per (asset,
...
void Composition::applyParameters() {
    ...
    // ADR-354: the authored lights' own parameters, into the scene copies `rebuild` made.
    //
...
nlohmann::json Composition::toJson() const {
    ...
    for (const auto& nodePtr : nodes_) {
        ...
        if (node.lod.enabled) { // ADR-348; written only when asked for, so nothing else grows a key
            json lod;
            lod["enabled"] = true;
...
Result<std::unique_ptr<Composition>> Composition::fromJsonImpl(const nlohmann::json& j,
    ...
    if (j.contains("nodes")) {
        ...
        for (const json& item : nodes) {
            ...
            if (item.contains("lod")) { // ADR-348: runtime LOD for this node's imported meshes
