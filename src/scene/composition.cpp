...
// ADR-344: this asset's LOD chains, built once and cached. The returned chains' `base` is the
...
void Composition::rebuild() {
    ...
    for (const auto& nodePtr : nodes_) {
        ...
        case NodeKind::Gltf: {
            ...
            // ADR-344: the coarser rungs, when this node asked for them. Built once per (asset,
...
nlohmann::json Composition::toJson() const {
    ...
    for (const auto& nodePtr : nodes_) {
        ...
        if (node.lod.enabled) { // ADR-344; written only when asked for, so nothing else grows a key
            json lod;
            lod["enabled"] = true;
...
Result<std::unique_ptr<Composition>> Composition::fromJsonImpl(const nlohmann::json& j,
    ...
    if (j.contains("nodes")) {
        ...
        for (const json& item : nodes) {
            ...
            if (item.contains("lod")) { // ADR-344: runtime LOD for this node's imported meshes
