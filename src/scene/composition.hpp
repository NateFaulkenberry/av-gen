...
// What a scene file says about runtime LOD on a Gltf node (ADR-344). Serialised as a `lod` block.
//
...
struct CompositionNode {
    ...
    // ADR-344: runtime LOD for this node's imported meshes. Off unless the scene file asks, which
...
private:
    ...
    // ADR-344: LOD chains built for imported assets, keyed by the asset path and the ladder asked
    ...
    // ADR-344: this asset's LOD chains, built once per (path, version, ladder) and shared.
