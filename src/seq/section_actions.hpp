...
//
// `section_direction.hpp` had a hole in it -- `SectionDirectionTable`, "the entire remaining job" --
...
//
//   1. a table an author can actually fill (`SectionDirectionSet`, below), and
...
// applies the result; it does not decide what a verb means, and neither does `section_direction.hpp`.
//
// ## Why the table is authored rather than built in
//
// `section_direction.hpp` warns that it "must not grow an opinion about the vocabulary of
...
//
// So the default still declines everything, and an authored `SectionDirectionSet` is what fills it.
...
struct SectionDirectionEntry {
    signals::MusicalSection kind = signals::MusicalSection::Intro;
    SectionDirection direction;
    [[nodiscard]] bool operator==(const SectionDirectionEntry&) const = default;
};
...
struct SectionDirectionSet {
    std::vector<SectionDirectionEntry> entries;
    ...
    [[nodiscard]] const SectionDirection* find(signals::MusicalSection kind) const;
    [[nodiscard]] bool empty() const { return entries.empty(); }
    [[nodiscard]] bool operator==(const SectionDirectionSet&) const = default;
};
...
[[nodiscard]] Result<void> validate(const SectionDirectionSet& set);

// ADR-225: a setting the application does not keep is not a setting.
[[nodiscard]] nlohmann::json toJson(const SectionDirectionSet& set);
[[nodiscard]] Result<SectionDirectionSet> sectionDirectionSetFromJson(const nlohmann::json& doc);

// The authored set as the table `generateDirectorEvents` takes. Declines exactly what the set does
// not mention, so an empty set behaves identically to `defaultSectionDirectionTable()`.
[[nodiscard]] SectionDirectionTable tableFrom(SectionDirectionSet set);
